# Supplied source register

This register records the material supplied for the first website research pass. Listing a source does not make every statement in it established fact or approved public copy.

## Health, co-operative and Aura planning

- *Health-Tech Cooperative Business Plan* - working business and access model.
- *Oceania Health and AI Surge Plan* - regional health and compute proposal.
- *Aged Care Act Adaptation and Health-Tech Plan* - aged-care pathway draft.
- *AURA GEODE to MACRO* - proposed Geode design and wider deployment model.
- *Clinical Research Path for Aura of Dementia* - proposed clinical research pathway.
- *Local Government Funding Inquiry Submission* - public investment and council funding argument.
- *Integrated Food System AI Expansion* - food-system and artificial-intelligence expansion proposal.
- *De-Slop Research for Community Understanding* - plain-language community framing research.

## Public submissions and regional pathways

- *Fiji-Australia Vuvale Union Submission, Luke Nathan Hayes, 2026*.
- *A Fair Go for the AI Age: Postcode-Level Sovereign Compute as Shared National Infrastructure*.
- *Do Not Put All Our Eggs in One Basket*.
- *Ocean of Peace Alliance: Veitacini Treaty Submission, Luke Nathan Hayes, 2026*.
- *World Cities 44,000 for AI Datacentre Committee* - large place list supporting a distributed-compute scenario.

## Research PDFs

- *AURA GEODE to Longevity* - compiled construction, personal atmosphere, sensory, biological-measure and digital-twin research directions. Its historical draft suggestions are source material, not instructions for this site.
- *82 Claytons Road Anglican Partnership Proposal* - proposed Amity place and partnership for care, reflection, research and community learning; no accepted transfer or operating partnership is implied.
- *Decentralised AI for Australian Communities*.
- *Decentralised Compute for Australian Communities*.
- *Architectural Blueprint for Oceania*.
- *Global Civic Tech Legislative Analysis*.
- *Legislative Research AI C-Hour*.
- *Straddie Insurance Branch Research Proposal*.

## Music and story

- *A Protopian Gambit* - 24-song lyric collection supplied for thematic review and later vertical video embeds.

## New connected civic source

- *C-Hour Joyful Responsible Abundance Plan* - voluntary contribution record and community gratitude. One C-hour records a verified hour of voluntary human contribution, with no monetary equivalent.

## Review labels

Public website material will use four visible labels:

- **Current source** - checked against a current primary or official source.
- **Working proposal** - a practical pathway offered for discussion and testing.
- **Future research** - a concept needing technical, clinical, legal or community development.
- **TO BE CONFIRMED** - a consequential detail still awaiting evidence or agreement.
